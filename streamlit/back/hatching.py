...
def _save_to_svg(input_png: str, w: int, h: int, vectors: Iterable[MultiLineString]) -> None:
    dwg = svgwrite.Drawing(input_png, size=(w, h), profile="tiny", debug=False)
...
def hatch(
    ...
    :param input_png: input image path
    :param output_svg: output svg path
    ...
    if save_svg:
        ...
        _save_to_svg(output_svg, img.shape[1], img.shape[0], [mls])

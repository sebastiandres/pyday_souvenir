...
text_1 = col1.text_input("Texto a reemplazar en svg", value="Hola")
text_2 = col2.text_input("Texto a reemplazar en svg", value="Mundo")
...
if col1.button("¿Cuánto tomará?"):
    status, draw_time = ah.draw_svg(updated_svg_file, speed_pendown, skip_draw=True)
    if status == "error":
        col1.error("Error conectando con AxiDraw")
    else:
        col1.info(f"Tiempo estimado: {draw_time:.1f} segundos")
if col2.button("¡Dibújalo!"):
    status, draw_time = ah.draw_svg(updated_svg_file, speed_pendown) # skip_draw=False by default
    if status == "error":
